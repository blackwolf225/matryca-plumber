...
**Matryca Plumber** is the definitive bridge between your trusted AI agent and your **Logseq OG** vault — a **headless CLI** and **MCP server** for safe read/write on Logseq's block tree (no raw Markdown parsing, no Logseq API, no silent overwrites), plus a **background daemon** and **Sovereign UI**. Built on [Andrej Karpathy's LLM-Wiki vision](https://karpathy.ai/blog). **Current: v1.10.6** — [`CHANGELOG.md`](CHANGELOG.md).
...
Seven MCP tools (`store_fact`, `ingest_document`, …) — `MATRYCA_MCP_ENABLED=true` when you trust the host. Spec: [`llms.txt`](llms.txt) · [`docs/openspec/agent-dx.md`](docs/openspec/agent-dx.md) · [`docs/openspec/agent-onboarding.md`](docs/openspec/agent-onboarding.md)
...
**Daemon** — semantic indexing, dangling links, entity consolidation, auto-split, `ingest_document`, link verification, LLM OS Soft Gate. OpenSpec: [`docs/openspec/README.md`](docs/openspec/README.md).

**Advanced `.env`** — copy [`.env.example`](.env.example); edge profile for large vaults: [`docs/v1.8-OPTIMIZATION-PLAN.md`](docs/v1.8-OPTIMIZATION-PLAN.md). Release history: [`CHANGELOG.md`](CHANGELOG.md).
...
| [`llms.txt`](llms.txt) | [`SECURITY.md`](SECURITY.md) |
