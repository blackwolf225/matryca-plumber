...
- **Tana import OpenSpec** — [`docs/openspec/tana-import.md`](docs/openspec/tana-import.md): enterprise migration contract for Tana workspace JSON → Logseq OG (`ijson` anti-OOM streaming, `logseq/config.edn` journal routing, depth-limit page-split, `tana-*` provenance).
- **Tana import streaming loader (WIP)** — `ijson` + `edn-format` dependencies; [`src/agent/importers/tana/load.py`](src/agent/importers/tana/load.py) streams `docs[]` without `json.load()`; [`src/graph/logseq_config.py`](src/graph/logseq_config.py) reads `:journal/page-title-format` from `logseq/config.edn`; [`src/agent/importers/tana/graph.py`](src/agent/importers/tana/graph.py) builds parent/child indexes and entity heuristics; [`src/agent/importers/tana/journal.py`](src/agent/importers/tana/journal.py) maps Java/EDN date patterns (including ordinal `do`) to journal titles and `journals/*.md` paths; [`src/agent/importers/tana/tags.py`](src/agent/importers/tana/tags.py) decodes field tuples (`SYS_D*`) into normalized Logseq properties with HTML label stripping and mega-tuple guards; [`src/agent/importers/tana/convert.py`](src/agent/importers/tana/convert.py) hybrid placement → `OutlineNode` plans with depth-split and `tana-*` provenance; [`src/agent/importers/tana/link.py`](src/agent/importers/tana/link.py) resolves wikilinks via in-flight page map + `MasterCatalog`; [`src/agent/importers/tana/write.py`](src/agent/importers/tana/write.py) idempotent OCC writes with `tana-id` pre-scan gate and `Tana/Import Log` ledger; [`src/agent/tana_import.py`](src/agent/tana_import.py) orchestrates load→write; CLI `matryca import tana --file … [--apply]`; MCP tool `import_tana`.
...
- **README** — Headroom-style rewrite: CLI/MCP bridge positioning, UI-first onboarding (no terminal `export` to start), progressive disclosure via `<details>`, compact docs table (~200 lines vs ~420).
