...
**Persistence (v1.9.10 unreleased — #41):** `_save_registry_unlocked` writes via `atomic_write_bytes` (temp → `fsync` → `os.replace`) so a crash mid-write cannot leave truncated JSON. Merge-on-save in `_persist_checked_registry_updates` reloads fresh entries under flock before applying verification deltas.
