...
- [x] **Link registry atomic save (v1.9.10 unreleased — #41)** — `_save_registry_unlocked` in `link_verification.py` uses `atomic_write_bytes` under existing `cross_process_json_flock`.
- [x] **Master catalog flock + merge-on-save (v1.9.10 unreleased — #35, #36)** — `load_master_catalog` reads under flock; `MasterCatalog.save()` merge-on-save by `last_mtime`.
- [x] **Harvest catalog/page parity (v1.9.10 unreleased — #37)** — Bootstrap skips catalog upsert when semantic index append OCC-aborts.
